...
export function InfoCard(props) {
  ...
  return (
    <Card className='wow fadeInUp bg-[#ffffff] dark:bg-zinc-850 text-black flex flex-col w-72 overflow-hidden relative'>
      {/* 信息卡牌第一行 */}
      ...
      <div className='flex justify-between'>
        <div className='flex space-x-3  hover:text-black dark:hover:text-white'>
          {/* 两个社交按钮 */}
          {url1 && (
            <div className='w-10 text-center bg-white p-2 rounded-full  transition-colors duration-200 dark:bg-zinc-850 dark:hover:bg-black hover:bg-white'>
              <Link href={url1}>
                <i className={icon1} />
              </Link>
            </div>
          ...
          {url2 && (
            <div className='bg-white p-2 rounded-full w-10 items-center flex justify-center transition-colors duration-200 dark:bg-zinc-850 dark:hover:bg-black hover:bg-white'>
              <Link href={url2}>
                <i className={icon2} />
              </Link>
            </div>
...
function GreetingsWords() {
  ...
  return (
    <div
      ...
      className=' select-none cursor-pointer py-1 px-2 bg-white hover:bg-black hover:text-indigo-950 dark:bg-zinc-850 dark:hover:text-white dark:hover:bg-black text-sm rounded-lg  duration-200 transition-colors'>
      {greeting}
    </div>
  )
}
